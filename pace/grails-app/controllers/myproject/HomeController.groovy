...
//            def calenderSpreadSymbol = ContractTable.executeQuery("Select distinct a.symbol from ContractTable  a where a.instrumentName in ('FUTIDX', 'FUTSTK') order by a.symbol")
            ...
            def calenderSpreadSymbol = abc.list {
                'in'("instrumentName",['FUTIDX', 'FUTSTK'])
                projections {
                    distinct("symbol")
                }
            }
            Collections.sort(calenderSpreadSymbol)
//
            println(calenderSpreadSymbol)
        ...
        def saveUserInput= {
            ...
            def returnMap=[:]
            def instrumentList = ['FUTIDX', 'FUTSTK']
            def token1 = ContractTable.findByInstrumentNameInListAndSymbolAndExpiryDate(instrumentList,params.calenderSpreadSymbol, Integer.parseInt(params.buyExpiryEpoch).toBigInteger()).token
            def token2 = ContractTable.findByInstrumentNameInListAndSymbolAndExpiryDate(instrumentList,params.calenderSpreadSymbol, Integer.parseInt(params.sellExpiryEpoch).toBigInteger()).token
            ...
            userInput.token1= token1
            userInput.token2=token2
            def strategy
            ...
            def userStrategyId = Sequence.getNextSequence('userStrategy_id')
            println('-------------------------'+userStrategyId)
            userInput.userStrategyId= userStrategyId
            if(userInput.save(flush: true, failOnError: true)){
                println(userInput.id)
                returnMap.saved=1
            }
        ...
        def saveUserStrategyInputs = {
            ...
            println('instance '+userInputTableIns)
            if(checkExistingValue(Integer.parseInt(inputs[0]), Integer.parseInt(inputs[1]),(int) Float.parseFloat(inputs[2])*100, 0, userInputTableIns)){
                ...
                if(Integer.parseInt(inputs[1])!=0 && params.statusPositive==(''+status.INACTIVE)){
                        ...
                        if(!existingStrategy){
                            ...
                            def strategyId = Sequence.getNextSequence('strategy_id')
                            println('-------------------------'+strategyId)
                            existingStrategy.strategy_id= strategyId
                            existingStrategy.save()
                        ...
                        def modId = Sequence.getNextSequence('mod_id')
                        println('-------------------------'+modId)
                        modStrategy.mod_id= modId
                        modStrategy.save()
             ...
             if(checkExistingValue(Integer.parseInt(inputs[3]), Integer.parseInt(inputs[4]), (int)Float.parseFloat(inputs[5])*100, 1, userInputTableIns)){
              ...
              if(Integer.parseInt(inputs[4])!=0 && params.statusNegative==(''+status.INACTIVE)){
                   ...
                   userInputTableIns.strategyIdNegative =  existingStrategy.strategy_id
                    ...
                    modStrategy.status= -1
                    modStrategy.user_id= springSecurityService.currentUser.id
                    println("this is the id of current user in negative "+ springSecurityService.currentUser.id)
                    def modId = Sequence.getNextSequence('mod_id')
                    println('-------------------------'+modId)
                    modStrategy.mod_id= modId
                    modStrategy.save()
...
//            //for nagetive flag = any non zero value
        ...
        def stopUserStrategyInputs(){
            ...
            userInputStopList.each{
                ...
                println('this is user input table '+ userInputTableIns.statusPositive)
                println('this is user input table '+ status.STARTED)
                if( userInputTableIns.statusPositive==(status.STARTED)){
                    println('status started now stoping')
